...
class ListItemValidatorWithMultipleValidatorsMapper implements Mapper
{
    ...
    public function map(mixed $data, ?MapperContext $context = null): array
    {
        ...
        foreach ($mapped as $index2 => $item2) {
            if ($item2 <= 0) {
                throw MappingFailedException::incorrectValue($item2, MapperContext::append($context, $index2), 'value greater than 0');
            }

            if ($item2 % 5 !== 0) {
                throw MappingFailedException::incorrectValue($item2, MapperContext::append($context, $index2), 'multiple of 5');
            }
        }
